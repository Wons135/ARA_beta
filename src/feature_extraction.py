...
# ---------- Add smoothed helpfulness ratio ----------
...
for df in [train_df, val_df, test_df]:
    ...
    df["smoothed_helpfulness_ratio"] = df.apply(
        lambda row: smoothed_helpfulness(row["helpful_vote"], row["rating"]) if row["rating"] > 0 else np.nan,
        axis=1
    )
    # Replace original target column
    df["helpfulness_ratio"] = df["smoothed_helpfulness_ratio"]
    df.drop(columns=["smoothed_helpfulness_ratio"], inplace=True)
...
def plot_target_distribution(df, target_column, set_name):
    ...
    plt.xlabel("Target Value (Helpfulness ratio)")
    plt.ylabel("Frequency")
    plt.grid(True)
    plt.show()

plot_target_distribution(train_df, "helpfulness_ratio", "Train")
plot_target_distribution(val_df, "helpfulness_ratio", "Validation")
plot_target_distribution(test_df, "helpfulness_ratio", "Test")
...
train_df[["helpfulness_ratio"]].to_csv("../datasets/preprocessed/y_train.csv", index=False)
val_df[["helpfulness_ratio"]].to_csv("../datasets/preprocessed/y_val.csv", index=False)
test_df[["helpfulness_ratio"]].to_csv("../datasets/preprocessed/y_test.csv", index=False)
