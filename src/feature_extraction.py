...
# Apply to all splits (only in case for regression
...
def plot_target_distribution(df, target_column, set_name):
    ...
    plt.xlabel("Target Value (Helpfulness class)")
    plt.ylabel("Frequency")
    plt.grid(True)
    plt.show()

plot_target_distribution(train_df, "helpfulness_class", "Train")
plot_target_distribution(val_df, "helpfulness_class", "Validation")
plot_target_distribution(test_df, "helpfulness_class", "Test")
...
train_df[["helpfulness_class"]].to_csv("../datasets/preprocessed/y_train.csv", index=False)
val_df[["helpfulness_class"]].to_csv("../datasets/preprocessed/y_val.csv", index=False)
test_df[["helpfulness_class"]].to_csv("../datasets/preprocessed/y_test.csv", index=False)
